...
from .utils import (make_tup_str, to_tensor, subsamplePC, fps_or_interpolate, rand_crop)
...
class ReIDDatasetBase(object):
    ...
    def evaluate(self,results,logger,neptune,**kwargs):
        ...
        to_delete = ['val_cls_gt','val_fp_gt','val_match_gt','val_vis_gt_all']
        ...
        print("IS val_vis_gt in RESULTS?: ", True if 'val_vis_preds' in results else False)
        if 'val_vis_preds' in results:
            print("VALVISGT: ", results['val_vis_gt'])
    ...
    def collect_dataset_idx(self,):
        ...
        total, tp, tp_tracked, fp, fp_tracked = 0, 0, 0, 0, 0
    ...
    def get_random_other_even_val(self,taken_idx,taken_cls, pts,i=None):
        """get a random index other than the one specified while ensuring 
        ...
        if np.random.choice([0,1]) == 1:
            ...
            if i is not None:
                i = np.random.randint(0, len(self.obj_tokens))
                other_token = self.obj_tokens[i]
                while(other_token == self.obj_tokens[i]):
                    other = np.random.choice(len(tps),1)[0] # Choose one object among class
            ...
            if i is not None:
                i = np.random.randint(0, len(self.obj_tokens))
                other_token = self.obj_tokens[i]
                # print("i is not None: ", other_token)
                while(other_token == self.obj_tokens[i]):
                    # print("self & other: ", self.obj_tokens[i], "&", other_token)
                    other = np.random.choice(len(fps),1)[0] # Choose one object among class
                    other_token = fps[other][0] # get token from tuple
            else:
                other_token = self.obj_tokens[taken_idx]
                # print("i is None: ", other_token)
                while(other_token == self.obj_tokens[taken_idx]):
                    other = np.random.choice(len(fps),1)[0] # Choose one object among class
                    other_token = fps[other][0] # get token from tuple
    ...
    def return_item(self,s1,s2,l1,l2,id1,id2):
        # print("\033[91mReIDDatasetBase return_item | s1 size: \033[0m", s1.shape)
        ...
        elif self.subsample_mode == "rand_crop":
            s1 = torch.tensor(rand_crop(s1, self.subsample_sparse))
            s2 = torch.tensor(rand_crop(s2, self.subsample_sparse))
        else:
            print("\033[91mSubsample Mode Must be among:\033[0m ['random', 'fps', 'rand_crop']")

        s1,s2,l1,l2,id1,id2 = DC(to_tensor(s1)),DC(to_tensor(s2)),DC(to_tensor(l1)),DC(to_tensor(l2)),DC(to_tensor(id1)),DC(to_tensor(id2))
        if self.return_mode == 'dict':
            return dict(sparse_1=s1, sparse_2=s2, label_1=l1, label_2=l2, id_1=id1, id_2=id2)
        elif self.return_mode == 'tuple':
            return s1,s2,l1,l2,id1,id2
        else:
            raise AttributeError(f"Invalid return mode {self.return_mode}")
        
    def return_item_size_dist(self,s1,s2,l1,l2,id1,id2,v1,v2):
        # print("\033[91mReIDDatasetBase return_item_size_dist | s1 size: \033[0m", s1.shape)
        # print("\033[91m[return_item_size_dist]subsample mode: \033[0m", self.val_subsample_mode)
        ...
        if self.val_subsample_mode == "random":
            ...
            s1 = torch.tensor(fps_or_interpolate(s1, self.subsample_sparse))
            s2 = torch.tensor(fps_or_interpolate(s2, self.subsample_sparse))
        elif self.val_subsample_mode == "rand_crop":
            s1 = torch.tensor(rand_crop(s1, self.subsample_sparse))
            s2 = torch.tensor(rand_crop(s2, self.subsample_sparse))
        else:
            print("\033[91mSubsample Mode Must be among:\033[0m ['random', 'fps', 'rand_crop']")
        ...
        s1,s2,l1,l2,id1,id2 = DC(to_tensor(s1)),DC(to_tensor(s2)),DC(to_tensor(l1)),DC(to_tensor(l2)),DC(to_tensor(id1)),DC(to_tensor(id2))
        if self.return_mode == 'dict':
            return dict(sparse_1=s1, sparse_2=s2, label_1=l1, label_2=l2, 
                        id_1=id1, id_2=id2, size_1=sz1, size_2=sz2,vis_1=vis1,vis_2=vis2)
        elif self.return_mode == 'tuple':
            return s1,s2,l1,l2,id1,id2,sz1,sz2,vis1,vis2
        else:
            raise AttributeError(f"Invalid return mode {self.return_mode}")
    ...
    def __getitem__(self,idx):
        ...
        if np.random.choice([0,1]) == 1:
            ...
            id2 = -1 if neg_obj_tok.startswith("FP") else self.instance_token_to_id[neg_obj_tok]
