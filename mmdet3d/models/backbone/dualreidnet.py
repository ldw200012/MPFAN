...
class ED_DualReID(nn.Module):
    def __init__(self, SA_conv_out=128, conv_out=128, nsample=[16,16,16], ED_conv_out=4):
        ...
        self.SA = PointTransformerBackbone(input_channels=0, use_xyz=True, conv_out=SA_conv_out, nsample=nsample)
        self.FE = DGCNN(dropout=0.5,emb_dims=1024, k=20, output_channels=40) # output = emb_dims = 1024

        # 1024 to 32 for DGCNN
        self.FE_conv1 = nn.Conv1d(1024, 256, 1)
        self.FE_conv2 = nn.Conv1d(256, 64, 1)
        self.FE_conv3 = nn.Conv1d(64, int(SA_conv_out/4), 1)

        self.FE_bn1 = nn.BatchNorm1d(256)
        self.FE_bn2 = nn.BatchNorm1d(64)
        self.FE_bn3 = nn.BatchNorm1d(int(SA_conv_out/4))
    ...
    def forward(self, pointcloud, numpoints):
        ...
        # Clone 1 through Self-Attention
        out1, h1 = self.SA(xyz, numpoints)     # h1 shape = [B, conv_out, N]
        
        # Clone 2 through DGCNN
        out2, h2 = self.FE(xyz.permute(0,2,1), numpoints)
