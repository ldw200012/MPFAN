...
def print_dataset_info(datasets, cfg, dataloader_kwargs):
    ...
    for i, dataset in enumerate(datasets):
        ...
        # Print training pair information
        if hasattr(dataset, 'idx') and hasattr(dataset, 'classes'):
            print(f"  • Training objects: {len(dataset.idx)}")
            print(f"  • Training pairs per epoch: {len(dataset.idx)} (1 pair per object per epoch)")
            
            # Calculate total possible positive pairs
            total_possible_pos_pairs = 0
            for obj_idx in dataset.idx:
                obj_tok = dataset.obj_tokens[obj_idx]
                num_frames = len(dataset.sparse_loader.obj_id_to_nums[obj_tok])
                if num_frames >= 2:
                    total_possible_pos_pairs += (num_frames * (num_frames - 1)) // 2
            
            print(f"  • Total possible positive pairs: {total_possible_pos_pairs:,}")
            print(f"  • Estimated total pairs per epoch: {len(dataset.idx) * 2} (including both positive and negative)")
            
            # Count objects per class
            unique_classes, class_counts = np.unique(dataset.classes, return_counts=True)
            print(f"  • Objects per class:")
            for cls_id, count in zip(unique_classes, class_counts):
                cls_name = dataset.idx_to_cls.get(cls_id, f"Class_{cls_id}")
