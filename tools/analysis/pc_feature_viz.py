...
def _class_dir_from_sample(sample: Dict[str, Any], dataset) -> str:
    """
    Determine output subfolder based on object class label in the sample.
    Uses dataset.idx_to_cls_fp to map label indices (including FP offsets) to names.
    Groups everything outside the tracked set into 'false_positives'.
    """
    # Extract integer class idx from sample['label_1'] if present
    cls_idx = None
    if isinstance(sample, dict):
        lbl = sample.get('label_1', None)
        if lbl is not None:
            try:
                # mmcv DataContainer or tensor or plain int
                if hasattr(lbl, 'data'):
                    v = lbl.data
                else:
                    v = lbl
    ...
    # Map to class name using dataset mapping that includes FP classes
    name = None
    try:
        name = dataset.idx_to_cls_fp.get(cls_idx, None) if hasattr(dataset, 'idx_to_cls_fp') else None
    except Exception:
        name = None

    # Allowed tracked classes for explicit folders
    allowed = {'car', 'truck', 'trailer', 'bus', 'motorcycle', 'pedestrian'}

    if not name:
        return 'false_positives'
    # FP classes are stored as 'FP_*'
    if isinstance(name, str) and name.startswith('FP'):
        return 'false_positives'
    if name in allowed:
        return name
    # Collapse all other classes into false positives bucket for this visualization need
    return 'false_positives'
...
def main():
    ...
    parser.add_argument('--out-dir', default='runs/pc_feature_viz_cls/pointtransformer')
    ...
    # Iterate all samples; start/end removed per request
    ...
    # Per-class quota control
    target_classes = ['car', 'truck', 'trailer', 'bus', 'motorcycle', 'pedestrian', 'false_positives']
    max_per_class = 30
    ...
    for idx in range(0, len(dataset)):
        ...
        # Determine class-specific output directory
        cls_dir = _class_dir_from_sample(sample if isinstance(sample, dict) else {}, dataset)
        # Skip if this class already met quota
        if cls_dir in class_counts and class_counts[cls_dir] >= max_per_class:
            continue
        out_dir_cls = osp.join(args.out_dir, cls_dir)
        tag = f'val_{idx}'
        ...
        if args.mode == 'magnitude':
            try:
                ...
                viz_magnitude(xyz, feat, raw_xyz, out_dir_cls, tag, args.open3d, args.point_size)
                # Count this successful sample toward the class quota
                if cls_dir in class_counts:
                    class_counts[cls_dir] += 1
                    # If all classes reached the quota, stop
                    if all(class_counts.get(c, 0) >= max_per_class for c in target_classes):
                        print(f"Reached {max_per_class} samples for all classes. Stopping.")
                        return
            ...
            try:
                if all(k in inputs for k in ['sparse_1', 'sparse_2']):
                    ...
                    if torch.is_tensor(s1) and torch.is_tensor(s2) and hasattr(model, 'siamese_forward'):
                        ...
                        viz_diff(xyz1_np, h1_np, xyz2_np, h2_np, raw_xyz1, raw_xyz2, out_dir_cls, tag, args.open3d, args.point_size)
                        if cls_dir in class_counts:
                            class_counts[cls_dir] += 1
                            if all(class_counts.get(c, 0) >= max_per_class for c in target_classes):
                                print(f"Reached {max_per_class} samples for all classes. Stopping.")
