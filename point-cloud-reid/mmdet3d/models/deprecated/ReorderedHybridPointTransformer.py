...
class ReorderedHybridPointTransformer(nn.Module):
    def __init__(self, input_channels, use_xyz, SA_conv_out, conv_out, k, normal_channel, nhead, attention):
        super(ReorderedHybridPointTransformer, self).__init__()
    ...
    def forward(self, pointcloud, numpoints):
        ...
        out1, h1 = self.sub1_SA(ordered_subset_1, numpoints)     # h1 shape = [B, conv_out, N]
        out2, h2 = self.sub2_PN(ordered_subset_2.permute(0,2,1), backbone_list=numpoints)    # h2 shape = [B, conv_out, N]
        ...
        return ordered_subset_2, z3 # [B, N/2, 3], [B, conv_out=64, N/2]
